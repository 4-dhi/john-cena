        ...
        h(function*() {
            ...
            if (f === 0)
                if (g.reportEarlySuccess) {
                    const k = nb(g);
                    k ? d(k) : (qb(a, g).catch(() => {}), c({
                        data: g.url
                    }))
                } else e = yield Z(a, g), "failureType" in e ? d(e) : c(e);
            else d({
                failureType: 8,
                command: f,
                ...
                return h(function*() {
                    ...
                    const T = M => h(function*() {
                        try {
                            ...
                            throw new sb(S.message);
                        }
                    });
                    if (!x) {
                        if (typeof y === "string") return yield T(y);
                        const M = y,
                            S = Object.keys(M).map(sa => h(function*() {
                                const tb = yield T(M[sa]);
                                return `${sa}=${tb}`
                            }));
                        return (yield Promise.all(S)).join("&")
                    }
                    return typeof y === "object" && y[x] ?
                        yield T(y[x]): m
                })
            }
...
var yb = function(a, b) {
    class c {
        ...
        I(d, e) {
            const f = d.clientId;
            if (d.type === 0) d.stats = vb(this.i, f, xb(this.g)), e(d);
